...
using HueCompanion.Views.Rendering;
...
public sealed partial class SceneBuilderPage : Page
{
    ...
    private (int r, int g, int b) HueColorToRgb(HueCompanion.Core.Models.HueColor color)
    {
    ...
    private async void GamutColorPicker_ColorChanged(object? sender, HueCompanion.Core.Models.HueColor color)
    {
    ...
    private void TransitionComboBox_SelectionChanged(object sender, SelectionChangedEventArgs e)
    {
        if (ViewModel?.SelectedKeyframe != null && TransitionComboBox.SelectedItem is ComboBoxItem item)
        {
            ...
            if (Enum.TryParse<HueCompanion.Core.Models.TransitionStyle>(tagValue, out var transition))
            {
