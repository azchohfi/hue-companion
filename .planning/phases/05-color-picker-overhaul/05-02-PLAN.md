...
must_haves:
  truths:
    - "ColorGamut.Contains() returns true for points inside gamut triangle"
    - "ColorGamut.Contains() returns false for points outside gamut triangle"
    - "ColorGamut.Clip() returns same point for points already inside gamut"
    - "ColorGamut.Clip() projects outside points to nearest edge"
    - "ColorGamut.GetMaxSaturationForHue() returns 1.0 for hues at gamut vertices"
  artifacts:
    - path: "src/HueCompanion.Core/Models/ColorGamut.cs"
      provides: "Gamut triangle math for Hue color constraints"
      exports: ["ColorGamut"]
      min_lines: 100
  key_links:
    - from: "ColorGamut.cs"
      to: "HueColor"
      via: "xy coordinate compatibility"
      pattern: "double [Xx], double [Yy]"
---

<objective>
Create ColorGamut class with gamut triangle algorithms.

Purpose: Provides the mathematical foundation for the gamut-aware color picker. This class encapsulates Philips Hue's color gamut triangles (A, B, C) and provides point-in-triangle testing, gamut clipping, and max saturation calculation per hue angle.

Output: New ColorGamut.cs file with complete gamut math, ready for use by GamutColorPicker control.
</objective>
...
@.planning/PROJECT.md
@.planning/ROADMAP.md
@.planning/research/HUE-COLORS.md

Gamut coordinates from research:
- GamutA: Red(0.704, 0.296), Green(0.2151, 0.7106), Blue(0.138, 0.08)
- GamutB: Red(0.675, 0.322), Green(0.4091, 0.518), Blue(0.167, 0.04)
- GamutC: Red(0.6915, 0.3083), Green(0.17, 0.7), Blue(0.1532, 0.0475)

D65 white point: (0.3127, 0.3290)

Key algorithms needed:
1. Point-in-triangle using cross product sign method
2. Closest point on line segment for gamut clipping
3. Ray-triangle intersection for max saturation calculation
</context>

<tasks>

<task type="auto">
  <name>Task 1: Create ColorGamut class with gamut definitions</name>
  <files>src/HueCompanion.Core/Models/ColorGamut.cs</files>
  <action>
Create new file `src/HueCompanion.Core/Models/ColorGamut.cs` with:
...
Ensure the file compiles with `dotnet build src/HueCompanion.Core/HueCompanion.Core.csproj`.
  </action>
  <verify>
`dotnet build src/HueCompanion.Core/HueCompanion.Core.csproj` succeeds.
File exists at `src/HueCompanion.Core/Models/ColorGamut.cs`.
  </verify>
  <done>ColorGamut class exists with static GamutA, GamutB, GamutC definitions</done>
</task>

<task type="auto">
  <name>Task 2: Add point-in-triangle and clipping methods</name>
  <files>src/HueCompanion.Core/Models/ColorGamut.cs</files>
  <action>
Add Contains() and Clip() methods to ColorGamut class:
...
- Contains(0.3127, 0.3290) should return true (white point is inside all gamuts)
- Contains(0.9, 0.9) should return false (outside CIE visible range)
- Clip(0.3127, 0.3290) should return (0.3127, 0.3290) unchanged
  </action>
  <verify>
`dotnet build src/HueCompanion.Core/HueCompanion.Core.csproj` succeeds.
  </verify>
  <done>ColorGamut has Contains() and Clip() methods for gamut boundary operations</done>
</task>

<task type="auto">
  <name>Task 3: Add max saturation per hue calculation</name>
  <files>src/HueCompanion.Core/Models/ColorGamut.cs</files>
  <action>
Add GetMaxSaturationForHue() method for adaptive saturation limits:
...
This provides GetMaxSaturationForHue() which will be used by the color picker to limit the saturation ring based on the current hue angle.
  </action>
  <verify>
`dotnet build src/HueCompanion.Core/HueCompanion.Core.csproj` succeeds.
  </verify>
  <done>ColorGamut has GetMaxSaturationForHue() for adaptive saturation limits</done>
</task>
...
Build Core project:
```
dotnet build src/HueCompanion.Core/HueCompanion.Core.csproj
```

Verify class structure:
```
grep -E "(public|private|static)" src/HueCompanion.Core/Models/ColorGamut.cs | head -20
```

Should show Contains, Clip, GetMaxSaturationForHue, and helper methods.
</verification>

<success_criteria>
1. ColorGamut.cs exists with ~150+ lines of code
2. Static GamutA, GamutB, GamutC definitions with correct coordinates
3. Contains() method for point-in-triangle testing
4. Clip() method for projecting out-of-gamut points to triangle edge
5. GetMaxSaturationForHue() method for calculating adaptive saturation limits
6. Build succeeds with no errors
</success_criteria>

<output>
After completion, create `.planning/phases/05-color-picker-overhaul/05-02-SUMMARY.md`
</output>
